...
function validateAndLogWarnings(config: unknown): void {
    ...
    for (const [key, field] of Object.entries(runtimeSchema.shape)) {
        const value = configObj?.[key];
        const schemaField = field as z.ZodType<any>;

        const hasDefault = schemaField instanceof z.ZodDefault;

        if (
            !hasDefault &&
            (value === undefined || value === null || value === "")
        ) {
            const description = (schemaField as any).description;
            ...
            if (typeof description === "string") {
                ...
                missingOrInvalidVars.push(`Missing: ${key}`);
            }
        } else if (
            value !== undefined &&
            value !== null &&
            ...
            const hasErrorForField = parsed.error.issues.some(
                (issue) => issue.path[0] === key,
            );
            if (hasErrorForField) {
                const error = parsed.error.issues.find(
                    (issue) => issue.path[0] === key,
                );
                missingOrInvalidVars.push(`${key}: ${error?.message}`);
            }
        }
    }

    if (missingOrInvalidVars.length === 0) {
        console.log("✓ All runtime environment variables are valid");
        return;
    }
    ...
    for (const warning of missingOrInvalidVars) {
        console.warn(`\n⚠ ${warning}`);
    }
...
export function validateRuntimeEnv(config: unknown): void {
    try {
        validateAndLogWarnings(config);
    } catch (error: unknown) {
        if (error instanceof Error) {
            console.warn(
                `⚠ Failed to validate environment variables: ${error.message}`,
            );
        }
    }
}
